...
void PlanningVisualization::drawViewpoints(const vector<Eigen::Vector3d>& points,
                                          const vector<Eigen::Vector3d>& views,
  ...
  // Draw viewpoints as spheres
  visualization_msgs::Marker sphere_marker;
  fillBasicInfo(sphere_marker, Eigen::Vector3d(0.2, 0.2, 0.2), color, ns, id,
                visualization_msgs::Marker::SPHERE_LIST);
  fillGeometryInfo(sphere_marker, points);
  marker_array.markers.push_back(sphere_marker);

  // Draw view directions as lines
  if (!views.empty()) {  // Only draw if view directions are provided
    visualization_msgs::Marker view_marker;
    fillBasicInfo(view_marker, Eigen::Vector3d(0.05, 0.05, 0.05), color, ns, id + 1, // Different ID
                  visualization_msgs::Marker::LINE_LIST);
    ...
    for (size_t i = 0; i < points.size(); ++i) {
      ...
      p2.y = views[i].y();
      p2.z = views[i].z();
      view_marker.points.push_back(p1);
      view_marker.points.push_back(p2);
    }
