...
void pointcloudInfoCallback(const sensor_msgs::PointCloud2ConstPtr &msg)
{
    ...
    // PointCloud<Normal>::Ptr cloud_normals_filtered(new PointCloud<Normal>);
    // cloud_filtered->reserve(cloud->size());    
    // cloud_normals_filtered->reserve(cloud->size()); 

    // for (size_t i = 0; i < cloud->size(); ++i) {
    //     Eigen::Vector3f normal(cloud_normals->points[i].normal_x,
    //                           cloud_normals->points[i].normal_y,
    //                           cloud_normals->points[i].normal_z);

    //     if (normal.norm() > 0 && (abs(normal.x()) > 1e-6 && (abs(normal.z()) > 1 || abs(normal.y()) > 1e-6) )) {
    //         cloud_filtered->push_back(cloud->points[i]);
    //         cloud_normals_filtered->push_back(cloud_normals->points[i]);
    //     }
