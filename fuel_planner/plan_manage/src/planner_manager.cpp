...
namespace fast_planner
{
                                          ...
                                          bool lookfwd, const double &relax_time)
  {
    ...
    const int num_ctrl_pts = local_data_.position_traj_.getControlPoint().rows(); // 控制点数量
    ...
    if (!has_rotated)
    {
      ROS_WARN("rotated dt_yaw:%.3lf", dt_yaw);
      const int num_yaw_pts = 36;
      double yaw_increment = 3 * M_PI / num_yaw_pts;
      double current_yaw = start_yaw[0];

      for (int i = 0; i < num_yaw_pts; ++i)
      {
        waypts.emplace_back(current_yaw, 0, 0);
        // 将偏航角约束均匀分配到控制点上
        int ctrl_pt_idx = i * (num_ctrl_pts - 1) / num_yaw_pts;
        waypt_idx.push_back(ctrl_pt_idx);
        current_yaw += yaw_increment;
        calcNextYaw(start_yaw[0], current_yaw);
      }
    }
    ...
    else if (lookfwd)
    {
      ...
      ROS_WARN("lookfd dt_yaw:%.3lf", dt_yaw);
    ...
    if (!has_rotated)
    {
      dt_yaw = local_data_.duration_ / 48; // time of B-spline segment
    ...
    {
      dt_yaw = local_data_.duration_ / 12;
      // Call B-spline optimization solver
      int cost_func = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::START | BsplineOptimizer::END |
                      BsplineOptimizer::WAYPOINTS;
      vector<Eigen::Vector3d> start = {Eigen::Vector3d(start_yaw3d[0], 0, 0),
