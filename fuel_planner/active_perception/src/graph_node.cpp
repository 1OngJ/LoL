...
double ViewNode::computeCost(const Vector3d& p1, const Vector3d& p2, const double& y1, const double& y2,
  ...
  double dist = ViewNode::searchPath(p1, p2, path);
  // double distance_penalty =  1.2* exp(dist * 0.01);
  double distance_penalty =  5.0*dist;
  ...
  // ROS_INFO("cost from computecost:%.3lf",max(pos_cost, yaw_cost) + distance_penalty);
  // return dist + max(pos_cost,yaw_cost);
