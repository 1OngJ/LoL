...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  finish_pub = nh.advertise<std_msgs::Bool>("/planning/finishexplore",10);
  explorefinish.data = false;

  replan_pub_ = nh.advertise<std_msgs::Empty>("/planning/replan", 10);
  new_pub_ = nh.advertise<std_msgs::Empty>("/planning/new", 10);
  bspline_pub_ = nh.advertise<bspline::Bspline>("/planning/bspline", 10);

  viewpoint_pub_ = nh.advertise<visualization_msgs::Marker>("/planning_vis/viewpoints", 10);
}
  ...
  switch (state_) {
    ...
    case PLAN_TRAJ: {
      ...
      if (res == SUCCEED) {
        ...
        explorefinish.data = true;
        finish_pub.publish(explorefinish);
...
void FastExplorationFSM::triggerCallback(const nav_msgs::PathConstPtr& msg) {
  ...
  // if (!has_rotated) {
  //   if (!planner_manager_->is_rotating) {
  //     planner_manager_->is_rotating = true;
  //     double current_yaw = fd_->odom_orient_.toRotationMatrix().eulerAngles(2, 1, 0)[0];
  //     planner_manager_->target_yaw = current_yaw + 2*M_PI/3;
  //     planner_manager_->calcNextYaw(current_yaw, planner_manager_->target_yaw);
  //     ROS_WARN("Start rotating.");
  //     transitState(ROTATING, "triggerCallback");
